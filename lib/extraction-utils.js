const PROVIDER_CONFIG = {
  claude: {
    ...
    selectors: {
      ...
      assistant: '.font-claude-response, [class*="font-claude-message"], .standard-markdown, .progressive-markdown, [data-testid="assistant-message"], [data-message-author-role="assistant"], [data-test-render="true"], .prose'
    }
  },
  chatgpt: {
    host: 'chatgpt.com',
    pathPrefix: '/share/',
    selectors: {
      user: '[data-message-author-role="user"], section[data-turn="user"], article[data-turn="user"], [data-testid^="conversation-turn-"][data-turn="user"]',
      assistant: '[data-message-author-role="assistant"], section[data-turn="assistant"], article[data-turn="assistant"], [data-testid^="conversation-turn-"][data-turn="assistant"]'
    }
  },
  gemini: {
    ...
    pathPrefix: '/share/',
    selectors: {
      user: 'share-turn-viewer user-query .query-text, share-turn-viewer user-query-content .query-text, share-turn-viewer .query-text, .share-turn-viewer .query-text, .query-text-line',
      assistant: 'share-turn-viewer response-container message-content .markdown-main-panel, share-turn-viewer response-container .markdown-main-panel, share-turn-viewer message-content .markdown-main-panel, .share-turn-viewer .markdown-main-panel, message-content .markdown-main-panel'
    }
  },
  grok: {
    host: 'grok.com',
...
function validateShareUrl(rawUrl) {
  ...
  // Gemini currently exposes both its canonical gemini.google.com/share URL
  // and public short-link hosts. Accept only their dedicated share paths so the
  // broader redirector host cannot be used as an arbitrary fetch target.
  if (hostname === 'g.co') {
    if (/^\/gemini\/share\/[^/?#]+\/?$/i.test(pathname)) {
      return { url: parsedUrl.toString(), parsedUrl, provider: 'gemini' };
    }
    return { error: 'Use a Gemini public link in the format https://g.co/gemini/share/...' };
  }
  if (hostname === 'share.gemini.google') {
    if (/^\/[^/?#]+\/?$/i.test(pathname)) {
      return { url: parsedUrl.toString(), parsedUrl, provider: 'gemini' };
    }
    return { error: 'Use a Gemini public link in the format https://share.gemini.google/...' };
  }
...
function parsePotentialStructuredBody(body) {
  ...
  const addCandidate = (value) => {
    ...
    if (!text || seen.has(text) || text.length > 8_000_000) return;
    seen.add(text);
  ...
  // Google RPC/batchexecute responses commonly start with an anti-XSSI prefix.
  addCandidate(original.replace(/^\)\]\}'\s*/, '').trim());

  // React/Next/Remix streaming responses can prefix JSON chunks with an id
  // such as `1:{...}` or `42:[...]`. Keep each parseable-looking chunk.
  for (const line of original.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    addCandidate(trimmed.replace(/^\d+\s*:\s*/, ''));
    const dataMatch = trimmed.match(/^data:\s*(.+)$/i);
    if (dataMatch && dataMatch[1] !== '[DONE]') addCandidate(dataMatch[1]);
  }
  ...
  const pushParsed = (value) => {
    let fingerprint;
    try { fingerprint = JSON.stringify(value).slice(0, 20000); } catch (_) { fingerprint = String(value); }
    if (parsedSeen.has(fingerprint)) return;
    parsedSeen.add(fingerprint);
    parsedValues.push(value);
  };

  for (const candidate of candidates) {
    try {
      pushParsed(JSON.parse(candidate));
      continue;
    } catch (_) {}

    // Some Google payloads are length-prefixed / line-delimited JSON. Pull out
    // complete JSON-looking lines without trying to interpret arbitrary HTML.
    for (const line of candidate.split(/\r?\n/)) {
      const value = line.trim().replace(/^\d+\s*:\s*/, '');
      if (!/^[\[{\"]/.test(value)) continue;
      try { pushParsed(JSON.parse(value)); } catch (_) {}
    }
  }
