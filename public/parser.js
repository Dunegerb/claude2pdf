(function () {
  ...
  function collectProviderScriptMessages(doc, htmlText, provider) {
    ...
    function walk(value, depth = 0) {
      ...
      if (typeof value === 'string') {
        ...
        if (trimmed.length <= 1000000 && /^[{[]/.test(trimmed)) {
          try { walk(JSON.parse(trimmed), depth + 1); } catch (_) {}
        } else if (trimmed.length <= 1000000 && /^data:/m.test(trimmed)) {
          trimmed.split(/\r?\n/).forEach((line) => {
            const match = line.match(/^data:\s*(.+)$/);
            if (!match || match[1] === '[DONE]') return;
            try { walk(JSON.parse(match[1]), depth + 1); } catch (_) {}
          });
        }
        return;
      }
  ...
  function parseChatGPTHTML(htmlText, sourceUrl = '') {
    ...
    return finalizeConversation({
      ...
      date: '',
      url: sourceUrl || getMeta(doc, 'link[rel="canonical"]', 'href'),
      messages: mergeProviderMessages(messages, mergeProviderMessages(collectNetworkFallbackMessages(doc), collectProviderScriptMessages(doc, htmlText, 'chatgpt')))
    });
  }
  ...
  function parseGeminiHTML(htmlText, sourceUrl = '') {
    ...
    return finalizeConversation({
      ...
      messages: mergeProviderMessages(messages, mergeProviderMessages(collectNetworkFallbackMessages(doc), collectProviderScriptMessages(doc, htmlText, 'gemini')))
    });
  }
  ...
  function parseGrokHTML(htmlText, sourceUrl = '') {
    ...
    return finalizeConversation({
      ...
      date: '',
      url: sourceUrl || getMeta(doc, 'link[rel="canonical"]', 'href'),
      messages: mergeProviderMessages(messages, mergeProviderMessages(collectNetworkFallbackMessages(doc), collectProviderScriptMessages(doc, htmlText, 'grok')))
    });
  }
